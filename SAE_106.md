...
La date limite de rendu est le mercredi 12/12 à 23h59. Le rendu se fera sur [l'espace Moodle du cours](https://moodle.umontpellier.fr/course/view.php?id=28316). Il faudra rendre dans un dossier compressé :
- les fichiers sources du site (dans une archive .zip),
- un fichier `readme.pdf` précisant:
   - les membres du groupe, leurs parts d'implication dans le projet,
   - le lien vers votre site web.
   - le lien vers le dépôt gitlab comportant les fichiers sources du site, attention à ajouter votre enseignant en tant que 'developer', vous complèterez aussi le fichier readme.md de ce dépôt avec le même contenu que ce fichier.

### Héberger le site

Le site devra être hébergé `page.html` sur le serveur webinfo. Les indications se trouvent dans les [consignes de la SAÉ 1.05](SAE_105.html)
