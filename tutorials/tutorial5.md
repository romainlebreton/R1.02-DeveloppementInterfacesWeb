...
Nous suivrons la présentation des fonctionalité faites dans [le système de grille à 12 colonnes](le système de grille à 12 colonnes).
...
sachant que la largeur totale est de 12 unitée. Selon la largeur totale de
...
**Exemple :** Le HTML suivant sert (en Foundation) à créer une boîte (class
...
précédement), pour découvrir les points de rupture de la page. Observez la
taille des colonnes varier selon la taille de l'écran.

2. Comment coderiez-vous cette fonctionalité ?

<!-- Il y a besoin de media queries et de définir la largeur en pourcentage du parent -->
...
colonnes selon si l'écran est `mall`, `medium` ou `large` ?
...
Nécessite de comprendre flex-basis : voir
